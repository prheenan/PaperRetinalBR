...
def run():
    ...
    xlim, _ = FigureUtil._limits(data)
    fmt = dict(xlim=xlim,ylim=[None,None])
    ax1 = plt.subplot(2,1,1)
    FigureUtil._plot_fec_list(data,color='k',**fmt)
    FigureUtil._plot_fec_list(data_sliced,**fmt)
    FigureUtil._plot_fmt(ax1, **fmt)
    ax2 = plt.subplot(2,1,2)
    plt.plot(landscape.q_nm-landscape_offset_nm,landscape.G0_kcal_per_mol)
    plt.plot(previous_JCP.q_nm-offset_jcp_nm,
             previous_JCP.G0_kcal_per_mol-offset_jcp_kcal,'r--')
    FigureUtil._plot_fmt(ax2, ylabel="G (kcal.mol)",is_bottom=True,**fmt)
